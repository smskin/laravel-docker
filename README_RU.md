...
- файлы `.gitlab-ci.simple.yml`и `.gitlab-ci.build-server.yml` -  используются для реализации CI через GitLab.
...
### Окружение Gitlab CI\CD
Это окружение используется для развертывания на сервере через механизмы GitLab CI.

В данном проекте есть 2 набора файлов:
- `./.gitlab-ci.build-server.yml` и `./docker/docker-compose.ci.build-server.yml` - для развертывания через Build сервере
- `./.gitlab-ci.simple.yml` и `./docker/docker-compose.ci.simple.yml` - для развертывания без Build сервера (сборка происходит на deploy сервере)

#### Отличия видов сборки:
В случае с `simple` сборка docker контейнеров происходит на deploy сервере. 
Из плюсов: проще инфраструктура
Из минусов: ресурсы production сервера используются для сборки

В случае с `build-server` следующая логика:
1. Запускается gitlab-runner build сервера (тег CI)
2. Собираются docker образы
3. Собранные образы отправляются в Docker репозиторий
4. Далее запускается gitlab-runner deploy сервера (тег CD_*)
5. Runner забирает новые образы из docker репозитория и обновляет текущие запущенные инстансы

Из плюсов: 
- сборка происходит на отдельном сервере, а значит не используются ресурсы production сервера для сборки контейнеров
- образы хранятся в репозитории
Из минусов: более сложная инфраструктура

#### Подготовка серверов для Build server сборки
1. Установите на Build server gitlab-runner и зарегистрируйте его с тегом `CI`
2. Установите на Deploy server gitlab-runner и зарегистрирйте его с тегом `CD_*` (например `CD_DEV_01` для Dev сервера и `CD_PROD_01` для Production сервера)
3. Настройте Docker registry (я использую Nexus CE)
4. Пропишите в Gitlab глобальные переменные: CI_DOCKER_REGISTRY, CI_DOCKER_REGISTRY_USER и CI_DOCKER_REGISTRY_PASSWORD (думаю что из названия понятно что за что отвечает)
5. В проекте пропишите переменную COMPOSE_PROJECT_NAME - это название проекта, которое будет использоваться в названии образов (например `test-project`)

#### Подготовка
1. Выберите подходящий вам вид сборки.
2. Переименуйте `/.gitlab-ci.*.yml` в `/.gitlab-ci.yml`
3. Переименуйте `./docker/docker-compose.ci.*.yml` в `./docker/docker-compose.ci.yml`

Далее в документации будет происходить обращение к `/gitlab-ci.yml` и `./docker/docker-compose.ci.yml`
